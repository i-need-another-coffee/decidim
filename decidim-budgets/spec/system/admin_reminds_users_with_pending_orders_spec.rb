...
describe "Admin reminds users with pending orders" do
  ...
  describe "create vote reminders" do
    ...
    it "sends reminders" do
      ...
      expect(last_email_first_link).to eq("http://#{organization.host}:#{Capybara.server_port}/en/processes/#{component.participatory_space.slug}/f/#{component.id}/budgets/#{budget.id}")
      expect(last_email_link).to eq("http://#{organization.host}:#{Capybara.server_port}/en/processes/#{component.participatory_space.slug}/f/#{component.id}/budgets")
    end
