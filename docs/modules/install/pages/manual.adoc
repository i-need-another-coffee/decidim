...
----
sudo apt update
sudo apt install -y build-essential curl git libssl-dev zlib1g-dev libffi-dev libyaml-dev libreadline6-dev
...
Now we are going to install redis server to help us with Asynchronous jobs like emails.
...
sudo apt-get update && sudo apt autoremove && sudo apt-get install -y nodejs
...
If you get an error like like the one below:
...
[source, bash]
----
sudo apt remove libnode72 nodejs
----

== 5. Installing Decidim
...
== 6.1. Initializing your app for local development
...
== 6.2. Initializing your application for production environment

=== 6.2.1. Additional environment variables
Assuming that you will not mix production with development environment, you may want to export the following env variables to your system:
[source,bash]
----
...
Run to get the new environment variables.

----
source /etc/environment
----

=== 6.2.2. Creating your secret_key_base

Any Rails application needs a `secret_key_base` parameter that is used to encrypt the session and the cookies. Run the below command to get a very long string, that you will need to add to your .rbenv-vars file under `SECRET_KEY_BASE` environment variable.
